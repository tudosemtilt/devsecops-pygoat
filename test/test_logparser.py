...
def test_return_raw_log_splitted_by_games():
  raw_test_log = """0:00 ------------------------------------------------------------
  0:00 InitGame: \\a\\huge\\path\\containg\\configs\\for\\this\\match
 15:00 A lot of stuff may have happened here
 20:37 ShutdownGame:
 20:37 ------------------------------------------------------------
  0:00 ------------------------------------------------------------
  0:00 InitGame: \\a\\huge\\path\\containg\\configs\\for\\this\\another\\match
 15:00 A lot of stuff may have happened here as well
 20:37 ShutdownGame:
 20:37 ------------------------------------------------------------"""

  games_log = LogParser.split_raw_log_into_games(raw_test_log)

  assert games_log == [
    """\\a\\huge\\path\\containg\\configs\\for\\this\\match
 15:00 A lot of stuff may have happened here
 20:37 ShutdownGame:
 20:37 ------------------------------------------------------------\n""",
    """\\a\\huge\\path\\containg\\configs\\for\\this\\another\\match
 15:00 A lot of stuff may have happened here as well
 20:37 ShutdownGame:
 20:37 ------------------------------------------------------------"""
  ]
...
def test_return_players_list_from_game_log():
  players = LogParser.get_players_list_from_game_log(SAMPLE_GAME_LOG)

  # Item order varies per run
  assert sorted(players) == sorted(['Isgalamido', 'Mocinha', 'Zeh', 'Dono da Bola'])


def test_return_kill_list_from_game_log():
  kills = LogParser.get_kills_list_from_game_log(SAMPLE_GAME_LOG)

  assert kills == [
    ('Isgalamido', 'Mocinha', 'MOD_ROCKET'),
    ('<world>', 'Zeh', 'MOD_TRIGGER_HURT'),
    ('<world>', 'Zeh', 'MOD_TRIGGER_HURT'),
    ('<world>', 'Dono da Bola', 'MOD_FALLING'),
  ]
